"""Pseudo-labeling: augment train with high-confidence test predictions, retrain CNN.

Colab-ready (CUDA). Reads stack_test probs → keeps samples above threshold →
retrains StrongCNN on combined set with K seeds. Loads .npy bundles directly.
"""
...
def main() -> None:
    ...
    p.add_argument("--stack-test-probs", type=Path, required=True,
                   help="npy of stack_test probs (from blend_all output)")
    ...
    print(f"pseudo: keeping {mask.sum()}/{len(mask)} test samples (thr={args.threshold})", flush=True)
